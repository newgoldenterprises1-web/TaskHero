...
async async function requestBookingCancellation(id){
  const booking=state.bookings.find(x=>x.id===id);
  if(!booking||['completed','cancelled','cancellation_requested'].includes(String(booking.status||'')))return;
  if(!confirm('Request cancellation for this booking?'))return;

  if(window.NearFamilyBackend?.ready){
    try{
      await NearFamilyBackend.requestBookingCancellation(id);
      booking.status='cancellation_requested';
      booking.cancellationRequestedAt=new Date().toISOString();
      booking.updatedAt=new Date().toISOString();
      save();
      renderAll();
      if($('bookingModal')?.classList.contains('show'))viewBooking(id);
      toast('Cancellation request submitted');
    }catch(e){
      console.warn(e);
      toast('Cancellation request could not be submitted');
    }
    return;
  }
  ...
  renderAll();
  if($('bookingModal')?.classList.contains('show'))viewBooking(id);
  toast('Cancellation request saved locally');
}
